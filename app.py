...
def validate_question_form(values):
    required_fields = [
        values["module_id"],
        values["text"],
        values["option_a"],
    ...
    if not all(required_fields):
        return "Please complete every question field."

    if values["correct_option"] not in {"A", "B", "C", "D"}:
        return "Please select A, B, C or D as the correct answer."

    try:
        module_id = int(values["module_id"])
    except ValueError:
        return "Please choose a valid module."

    if not db.session.get(Module, module_id):
        return "The selected module could not be found."
...
def home():
    ...
    <section class="hero">
        ...
        <p>
            This assessment checks your understanding of customer calls,
            locksmith services, pricing, location capture, dispatch and escalation.
        </p>

        <p class="muted">
            Enter your details to begin. Your results will be saved for your trainer.
        </p>
...
def modules():
    ...
    module_list = Module.query.order_by(
        Module.position,
        Module.id,
    ).all()
    ...
    <section class="hero">
        ...
        <p>
            Select a module below. The pass mark is {{ pass_mark }}%.
        </p>
...
def take_quiz(module_id):
    ...
    if request.method == "POST":
        ...
        <section class="hero result">
            ...
            {% if passed %}
                <p>
                    You have passed this knowledge check.
                    This contributes to, but does not replace, trainer sign-off.
                </p>
            {% else %}
                <p>
                    Review the explanations below, then retake the module.
                    The pass mark is {{ pass_mark }}%.
                </p>
            {% endif %}
...
def admin_login():
    ...
    <section class="hero">
        ...
        <p>
            Enter the manager password to view results
            and manage the Academy.
        </p>
...
def admin_dashboard():
    ...
    unique_students = db.session.query(
        func.count(
            func.distinct(Attempt.student_email)
        )
    ).scalar() or 0
    ...
    content = (
        ...
        <section class="hero">
            ...
            <p>
                Review module attempts and identify students
                who need further coaching.
            </p>
                ...
                <table>
                    ...
                    <tbody>
                        {% for attempt in attempts %}
                            <tr>
                                <td>
                                    {{ attempt.created_at.strftime('%d %b %Y %H:%M') }}
                                </td>
...
def admin_questions():
    ...
    content = (
        ...
        <section class="hero">
            ...
            <p>
                Edit existing questions, change answers,
                add new questions or remove questions you no longer need.
            </p>

            <a class="btn" href="{{ url_for('admin_add_question') }}">
                Add new question
            </a>
        </section>
        ...
        {% for module in modules %}
            <section class="card" style="margin-bottom:22px;">
                ...
                {% else %}
                    <p>No questions have been added to this module yet.</p>
                {% endif %}
            </section>
        {% endfor %}
        """
...
<section class="hero">
    ...
    <p>
        ...
        The correct answer must match one of the four answer options.
    </p>
    ...
    <form method="post">
        ...
        <select id="module_id" name="module_id" required>
            ...
            {% for module in modules %}
                <option
                    value="{{ module.id }}"
                    {% if selected_module_id|string == module.id|string %}
                        selected
                    {% endif %}
                >
        ...
        <label for="explanation">Explanation shown after marking</label>
...
def admin_add_question():
    ...
    if request.method == "POST":
        values = get_question_form_values()
        error = validate_question_form(values)
    ...
    return page(
        ...
        modules=modules,
        values=values,
        selected_module_id=values["module_id"],
    )
...
def admin_modules():
    ...
    content = (
        ...
        <section class="hero">
            ...
            <p>
                Change module titles, descriptions and display order,
                or add a new module.
            </p>

            <a class="btn" href="{{ url_for('admin_add_module') }}">
                Add new module
            </a>
        </section>
            ...
            {% for module in modules %}
                <article class="card">
                    ...
                    <div class="actions">
                        <a
                            class="btn small"
                            href="{{ url_for('admin_edit_module', module_id=module.id) }}"
                        >
                            Edit module
                        </a>
                        ...
                        {% if module.questions|length == 0 %}
                            <form
                                method="post"
                                action="{{ url_for('admin_delete_module', module_id=module.id) }}"
                                onsubmit="return confirm('Delete this empty module?');"
                            >
                                <button
                                    class="btn small danger"
                                    type="submit"
        ...
        <div class="danger-box">
            ...
            A module cannot be deleted while it contains questions.
            Move or delete those questions first.
        </div>
        """
    )
...
def admin_add_module():
    ...
    if request.method == "POST":
        ...
        if not all(values.values()):
            flash("Please complete every module field.")
        else:
            try:
...
def admin_delete_module(module_id):
    ...
    if module.questions:
        flash(
            "This module still contains questions. "
            "Move or delete them before deleting the module."
        )
        return redirect(url_for("admin_modules"))
    ...
    if attempt_count:
        flash(
            "This module has historic student results "
            "and cannot be deleted safely."
        )
        return redirect(url_for("admin_modules"))
...
def seed_data():
    ...
    modules_data = [
            ...
            "questions": [
                {
                    ...
                    "correct": "B",
                    "explanation": (
                        "The caller type affects authority, "
                        "payment responsibility and the questions "
                        "the agent must ask."
                    ),
                },
                {
                ...
                },
                {
                    ...
                    "correct": "C",
                    "explanation": (
                        "The full postcode is essential for checking "
                        "coverage, choosing a technician and quoting "
                        "a realistic ETA."
                    ),
                },
                {
                    ...
                    "explanation": (
                        ...
                        "a clear and professional booking attempt."
                    ),
                },
                {
                    "text": (
                    ...
                    "options": [
                        ...
                        (
                            "Confirm the customer, address, problem, "
                    ...
                    "correct": "B",
                    "explanation": (
                        "A concise confirmation reduces mistakes "
                        "and gives the technician complete information."
                    ),
                },
            ],
        },
        {
            ...
            "description": (
                "Common enquiries, services offered, "
                "excluded work and safe use of terminology."
            ),
            "position": 2,
            "questions": [
                {
                    ...
                    "explanation": (
                        ...
                        "Safe combinations, access control "
                        "and fob systems are not."
                    ),
                },
                {
                    "text": (
                    ...
                    "options": [
                        ...
                        (
                            "Promise that no replacement "
                            "will be needed"
                        ),
                    ],
                    "correct": "C",
                    "explanation": (
                        "Clear description is safer than an incorrect "
                        "diagnosis. The technician can assess the lock "
                        "on arrival."
                    ),
                },
                {
                    ...
                    "correct": "B",
                    "explanation": (
                        "The distinction affects urgency, "
                        "safety questions and dispatch notes."
                    ),
                },
                {
                    "text": (
                        "Which brand may commonly be mentioned "
                        "on a locksmith call?"
                    ),
                    "options": [
                        "Banham",
                    ...
                    "correct": "A",
                    "explanation": (
                        "Banham is one of the lock brands agents "
                        "may hear, alongside Yale, Ingersoll, "
                        "Chubb and Assa Abloy."
                    ),
                },
                {
                    "text": (
                    ...
                    "options": [
                        ...
                        (
                            "Understand enough to ask useful questions "
                    ...
                    "correct": "B",
                    "explanation": (
                        "Call handlers gather information; "
                        "technicians diagnose and complete "
                        "the physical work."
                    ),
                },
            ],
        },
        {
            ...
            "description": (
                "Correct wording, realistic expectations, "
                "vulnerable situations and difficult calls."
            ),
            "position": 3,
            "questions": [
                {
                    ...
                    "options": [
                        ...
                        (
                            ...
                            "depending on assessment and work required"
                        ),
                    ...
                    "correct": "B",
                    "explanation": (
                        "The agent should explain the starting price "
                        "accurately without promising a final figure "
                ...
                },
                {
                    ...
                    "options": [
                        (
                            "The shortest time the customer "
                            "wants to hear"
                        ),
                    ...
                    "explanation": (
                        ...
                        "The nearest technician by distance "
                ...
                },
                {
                    ...
                    "options": [
                        ...
                        (
                            "Recognise the vulnerability and escalate "
                            "or prioritise according to procedure"
                        ),
                    ...
                    "correct": "B",
                    "explanation": (
                        "Vulnerable-person situations require careful "
                        "questioning, urgency and escalation according "
                        "to company procedure."
                    ),
                },
                {
                    "text": (
                        "A caller cannot show that they have permission "
                        "to enter a property. What should the agent do?"
                    ),
                    "options": [
                        "Dispatch without mentioning it",
                        (
                            "Ignore the issue if they "
                            "sound confident"
                        ),
                        (
                            "Escalate the concern and do not "
                    ...
                    "correct": "C",
                    "explanation": (
                        "Authority and ownership concerns must be "
                        "escalated rather than ignored."
                    ),
                },
                {
                    "text": (
                        "What is the best approach when the agent "
                    ...
                    "correct": "C",
                    "explanation": (
                        "A good agent follows the process and seeks "
                        "support instead of giving inaccurate information."
                    ),
                },
            ],
        },
